...
% --------------------------------------------------------   
    ...
    imdbs = imdbs(:);
    roidbs = roidbs(:);
    %liu@0819 added 'image_scale' field
        ...
        image_roidb = ...
            cellfun(@(x, y) ... // @(imdbs, roidbs)
                arrayfun(@(z) ... //@([1:length(x.image_ids)])
                    ...
                    'boxes', y.rois(z).boxes(y.rois(z).gt, :), 'class', y.rois(z).class(y.rois(z).gt, :), 'image', [], 'bbox_targets', [], 'image_scale', []), ...
        ...
        image_roidb = ...
            cellfun(@(x, y) ... // @(imdbs, roidbs)
                arrayfun(@(z) ... //@([1:length(x.image_ids)])
                    ...
                    'boxes', y.rois(z).boxes, 'class', y.rois(z).class, 'image', [], 'bbox_targets', [], 'image_scale', []), ...
                [1:length(x.image_ids)]', 'UniformOutput', true),...
            imdbs, roidbs, 'UniformOutput', false);%'
    end
   %liu@0801: change from a cell to struct
...
function [image_roidb, means, stds] = append_bbox_regression_targets(conf, image_roidb, means, stds)
    ...
    image_scale = cell(num_images, 1);
    %parfor i = 1:num_images
    %liu@0811: change to for to make debugging easier
    for i = 1:num_images
       ...
       image_scale{i} = cell2mat(im_scales);
    end
    clear image_roidb_cell;
    for i = 1:num_images
        image_roidb(i).bbox_targets = bbox_targets{i};
        image_roidb(i).image_scale = image_scale{i};
    end
    clear bbox_targets;
        ...
        for i = 1:num_images
           for j = 1:length(conf.scales)
                ...
                if ~isempty(gt_inds)
                    ...
                    % liu@0810: make sure no complex numbers involved
                    assert(isreal(targets(gt_inds, 2:end)), 'Target coordinate values must be real.\n');
...
function scaled_rois = scale_rois(rois, im_size, im_scale)
    %liu@0810: all roi coordinates should be >= 1
    rois_tmp = max(1, rois);
    %liu@0810: check invalid ground-truth rois --> any elements in [x1 y1 x2 y2] is <= 0
    invalid_idx = (rois_tmp(:,3) <= rois_tmp(:,1)) | (rois_tmp(:,4) <= rois_tmp(:,2));

    im_size_scaled = round(im_size * im_scale);
    % liu@0802:scale(1) -- height ratio,  scale(2) -- width ratio
    scale = (im_size_scaled - 1) ./ (im_size - 1);
    % liu@0802: rois(1) and (3) are related to width, aka scale(2), rois(2) and (4) are related to height, aka scale(1)
    scaled_rois = bsxfun(@times, rois_tmp - 1, [scale(2), scale(1), scale(2), scale(1)]) + 1;
    
    if sum(invalid_idx) ~= 0
        scaled_rois(invalid_idx, :) = [];
        fprintf('Invalid coordinates are deleted.\n');
    end
end
...
%   negative: [-1, zero(regression_label)]
    ...
    % as (gt_best_matches)
    % liu@0802: there may be more than one anthors that achieve max IOU with a gt bbox, so find them all
    %liu@0802:gt_best_matches is the row index, gt_ind is the column index, putting them together can find the exact location
    ...
    %liu@0802:conf.fg_thresh==0.7, find all anchors with a IOU with gt equal or larger than 0.7
    ...
    % pursue high recall
    %liu@0802: conf.bg_thresh_hi == 0.3,  conf.bg_thresh_lo==0
    bg_inds = setdiff(find(ex_max_overlaps < conf.bg_thresh_hi & ex_max_overlaps >= conf.bg_thresh_lo), ...
                    fg_inds);
    %liu@0802: make sure all the bgs and fgs are within the image region
    if conf.drop_boxes_runoff_image
...
function contained = is_contain_in_image(boxes, im_size)
    %liu@0802: boxes[x, :] = [x1 x2 x3 x4], x1 and x3 can be as long as width (= im_size(2)), while  x2 and x4 can be as long as height (= im_size(1))
    contained = boxes >= 1 & bsxfun(@le, boxes, [im_size(2), im_size(1), im_size(2), im_size(1)]);
    %liu@0802: true if all elements of a vector are nonzero
