function net = rpnInitializeModel_conv4(varargin)
...
%only keep layers from conv1_1 to relu4_3, remove fully connected layers
net.layers = net.layers(1:23);  %1:30
...
net.addLayer('conv_proposal1', ...
              dagnn.Conv('size', [3 3 512 512],...
                          ...
                          'stride', [1, 1],...
                          'pad', [1 1 1 1]), ...
              {'x23'}, {'conv_proposal1'}, ...  %{input layer} + {output layer(generally same as this layer's name)}
...
net.vars(27).precious = true;  % proposal_cls_score  %34 --> ?
net.vars(29).precious = true;  % label   % 36 --> ?
net.vars(30).precious = true;  % loss_cls % 37 --> ?
net.vars(34).precious = true;  % loss_bbox % 41 --> ?